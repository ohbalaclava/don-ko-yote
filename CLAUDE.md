...
- `data/metronome.js` (pure, unit-tested) — the optional playback metronome's beat-grid logic. `JIUCHI_PATTERNS` maps each jiuchi to the 1-indexed subdivisions it ticks within a beat; `metronomeTicks(totalDiv, time, { positions, headOnly, emphasise })` returns the ordered `{ div, accent }` ticks.

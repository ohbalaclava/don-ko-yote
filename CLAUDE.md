...
- `piece.js` — singleton mutable state for the current piece. Shape: `{ id, title, jiuchi, beatsPerLine, bpm, author, icon, showVolume, metronome, metronomeHeadOnly, metronomeEmphasiseHead, metronomeJiuchi, metronomeShime, metronomeVolume, selectedLineId, editingTile, selectMode, selection, lines: [{ id, sounds: [], repeat: number }] }`. The `metronome*` fields are the per-score metronome config (see `PERSISTED_FIELDS`). Sounds are either sound objects `{ id, name, hand, duration, instruction }` or group objects `{ id, type: 'group', name, sounds[], duration }`. All mutations go through methods on `piece` (e.g. `piece.addSound`, `piece.moveSound`, `piece.addGroup`, `piece.expandGroup`) which push to history and call `m.redraw()`. Select mode tracks `{ lineId, anchorId, soundIds[] }` for anchor-based contiguous range selection.
...
- `settings.js` — singleton for app-level preferences (`proportionalWidth`, `font`, `darkMode`, `defaultBackground`, `defaultAuthor`, `defaultShowVolume`, `countIn`, master `volume`). Persisted to IndexedDB via `db.kv`. `settings.set(key, value)` persists and redraws. `defaultBackground` is a data URL that serves as a fallback when a score has no background image set. Metronome config is **not** here — it is stored per-score on `piece`.
...
- `AddRowActions.jsx` — the contextual "+ Line / Heading / Note / Divider / Jiuchi" toolbar. The `piece.add*` methods insert the new row immediately after the selected line, so it renders inside the selected `Line` (and additions land where the user is working).
